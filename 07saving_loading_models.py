...
def save_checkpoint(epoch, state, filename="checkpoint.pth"):
...
batch_size = 128
num_epochs = 8
load_model = 1  # will trigger load_checkpoint() function if set to true
...
# Loading model if checkpoint saved
if load_model:
    load_checkpoint(checkpoint=torch.load("checkpoints/epoch8_checkpoint.pth"),  # when checkpoint found, is loaded
                    # and training starts from that checkpoint / state of the model
                    model=model,
                    optimizer=optimizer
                    )
...
for epoch in range(num_epochs):
    ...
    print(f"\nloss at epoch {epoch + 1}: {mean_loss:.6f}")
